...
def get_cities_links(url):
    ...
    for city, link in zip(cities_names, cities_links):
        cities[link.get_text()] = (
            city.get_text(),
            link.find_all("a", href=True)[0]["href"],
        )  # The key is the city code
    ...
    for item in results:
        parties.update(
            [
                (k, n[0])
                for k, n in zip(
                    results[item]["parties_votes"].keys(),
                    results[item]["parties_votes"].values(),
                )
            ]
        )
    ...
    for city in results:
        for party in sorted(parties):  # order parties by party number
            if party[0] not in results[city]["parties_votes"]:
                results[city]["parties_votes"][party[0]] = (
                    party[1],
                    0,
                )  # add missing party with 0 votes
...
def get_city_election_results(base_url, cities):
    ...
    for city, (city_name, link) in cities.items():
        ...
        registered = int(
            city_content.find_all("td", {"class": "cislo", "headers": "sa2"})[0]
            .get_text()
            .replace("\xa0", "")
        )
        envelopes = int(
            city_content.find_all("td", {"class": "cislo", "headers": "sa3"})[0]
            .get_text()
            .replace("\xa0", "")
        )
        valid = int(
            city_content.find_all("td", {"class": "cislo", "headers": "sa6"})[0]
            .get_text()
            .replace("\xa0", "")
        )
        ...
        party_number = city_content.find_all(
            "td", {"class": "cislo", "headers": re.compile(r"t[1|2]sa1 t[1|2]sb1")}
        )
        party_name = city_content.find_all(
            "td",
            {"class": "overflow_name", "headers": re.compile(r"t[1|2]sa1 t[1|2]sb2")},
        )
        total_votes = city_content.find_all(
            "td", {"class": "cislo", "headers": re.compile(r"t[1|2]sa2 t[1|2]sb3")}
        )

        for number, party, votes in zip(party_number, party_name, total_votes):
            parties[int(number.get_text().replace("\xa0", ""))] = (
                party.get_text(),
                int(votes.get_text().replace("\xa0", "")),
            )
...
def main(url, filename):
    ...
    city_results = get_city_election_results(
        base_url, cities
    )  # Pass base_url and cities {"code": (name, link)} to fetch data of city pages
